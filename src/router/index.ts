...
import { systemRoutes, systemAuthRoutes } from './modules/system'  // router>modules>system.ts模块[系统login、404、layout等路由]
import menuTool from "@/utils/menuRoute"

//扁平化本地静态路由(配置在router>modules>*.ts)
...
router.beforeEach((to, from, next) => {
    ...
    // 3.判断是否访问的是静态auth路由，有 Token 就在当前页面，没有 Token 重置路由到登陆页
