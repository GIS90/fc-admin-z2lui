          ...
          <el-row :gutter="40" class="main-box">
            ...
            <el-col :lg="12">
              ...
              <el-form-item label="名称" prop="name">
                ...
                />
                <div class="el-form-item-msg">系统唯一且与内置组件名一致，建议全英文驼峰写法，示例：SystemMainMenu</div>
              </el-form-item>
              ...
              <!--如果是新增、父节点可选；如果是新增子节点，禁用父节点选择-->
              <el-form-item label="上级菜单" prop="pid">
                <el-select v-if="props.pid === undefined" v-model="ruleForm.pid" style="width: 100%;" :size="selectSingleField.size" :clearable="selectSingleField.clearable"
                ...
                <el-select v-else v-model="ruleForm.pid" style="width: 100%;" :size="selectSingleField.size" :clearable="selectSingleField.clearable"
                           :filterable="selectSingleField.filterable" :allow-create="selectSingleField.allowCreate"
                           :no-match-text="selectSingleField.noMatchText" :no-data-text="selectSingleField.noDataText"
                           :placeholder="ProfilePlace.pid" disabled>
                  <el-option-group v-for="group in menuOption" :key="group.label" :label="group.label">
                    <el-option v-for="item in group.options" :key="item.value" :label="item.label" :value="item.value">
                      <span class="el-select-key">{{ item.label }}</span>
                      <span class="el-select-value">{{ item.name }}</span>
              ...
              <el-form-item label="路由地址" prop="path">
                <el-input v-model.trim="ruleForm.path"
                          ...
                          :disabled="btnDisabled"
                />
                <div class="el-form-item-msg">配置的菜单路由地址，以/开头，示例：/system/main/menu</div>
              </el-form-item>
...
import {SysMainMenuAddApi, SysMainMenuAddEnumApi} from "@/api/modules/system";
...
const resetForm = () => {
  ruleForm.path = "";
  ruleForm.name = "";
  ruleForm.pid = props.pid;
  ruleForm.redirect = "";
  ruleForm.component = "";
  ruleForm.level = undefined;
  ruleForm.order_id = 1;
  ruleForm.icon = "";
  ruleForm.title = "";
  ruleForm.type = "MENU";
  ruleForm.isHide = false;
  ruleForm.isFull = false;
  ruleForm.isAffix = false;
  ruleForm.isKeepAlive = true;
  ruleForm.isBreadcrumb = true;
  ruleForm.tag = "";
};

// 请求数据API
const fetchApiData = async () => {
  const response = await SysMainMenuAddEnumApi();
  if (response.data) {
...
const confirm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid) => {
    if (valid) {
      ...
      SysMainMenuAddApi(ruleForm).then(() => {
        ElNotification({
          title: '提示',
          message: '新增成功',
          type: 'success'
