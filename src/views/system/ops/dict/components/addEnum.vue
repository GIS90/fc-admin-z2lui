      ...
      <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules"
        ...
        <el-form-item label="字典枚举Key值" prop="name">
          <el-select v-model="ruleForm.name" style="width: 100%;" :size="selectSingleField.size" :clearable="selectSingleField.clearable"
                     ...
                     :placeholder="ProfilePlace.name" :disabled="props.name !== ''">
...
import { DictEnumBaseModel } from "@/models/systemModel";
import {SysOpsDictEnumAddApi, SysOpsDictEnumAddInitApi} from "@/api/modules/system";
...
const ruleForm = reactive({
  ...
  description: "",
  order_id: 1
}) as DictEnumBaseModel;

// from rule
const rules = reactive<FormRules<DictEnumBaseModel>>({
  name: [
...
// 监听 props.name 变化，自动更新 ruleForm.name
watch(() => props.name, (newName) => {
  ruleForm.name = newName;
});
...
const fetchApiData = async (md5: string) => {
  const response = await SysOpsDictEnumAddInitApi();
  if (response.data) {
    const res = response as { data: { key: OptionEnumModel[] } }; // declare
...
const confirm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid) => {
    if (valid) {
      ...
      SysOpsDictEnumAddApi(ruleForm).then(() => {
        ElNotification({
          title: '提示',
          message: '新增成功',
          type: 'success'
