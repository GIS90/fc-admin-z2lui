...
export const useTable = (
    ...
    // 初始化显示条数从缓存获取，IF不存在直接使用默认系统配置
    const useTablePageSize = settingStore.getTablePageSize || tablePageSize;
    ...
    const handleSizeChange = (val: number) => {
        ...
        settingStore.setTablePageSize(val); // 更新STORE缓存
        getTableList();
    };
